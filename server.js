...
app.get('/viestit', requireLogin, (req, res) => {
    haeViestit((err, viestit) => {
        if (err) {
            ...
            return res.render('viestit', { viestit: [], error: res.locals.t.messages.fetchError });
        }
...
app.post('/viestit', requireLogin, (req, res) => {
    ...
    if (!viesti) {
        return haeViestit((err, viestit) => {
            res.render('viestit', { viestit: viestit || [], error: res.locals.t.messages.empty });
        });
    }
    ...
    db.exec(sql, (err) => {
        ...
        if (err) {
            ...
            return haeViestit((fetchErr, viestit) => {
                res.render('viestit', { viestit: viestit || [], error: res.locals.t.messages.insertError });
            });
        }
        res.redirect(res.locals.p('/viestit'));
    });
...
app.get('/kayttajat', requireLogin, (req, res) => {
    ...
    db.all('SELECT * FROM kayttajat', [], (err, kayttajat) => {
        ...
        if (err) {
            ...
            return res.render('kayttajat', { kayttajat: [], error: res.locals.t.users.fetchError });
        }
...
app.get('/tapahtumat', requireLogin, (req, res) => {
    const db = connectDB();
    db.all('SELECT * FROM tapahtumat ORDER BY aikaleima DESC, id DESC', [], (err, tapahtumat) => {
        db.close();
        if (err) {
            console.error('Virhe haettaessa tapahtumia:', err);
            return res.render('tapahtumat', { tapahtumat: [], error: res.locals.t.events.fetchError });
        }
